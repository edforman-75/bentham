#!/usr/bin/env npx tsx
/**
 * Run HUFT 100-Prompt Study - Google Search from India (Bangalore)
 *
 * Different city = different IP pool to avoid Mumbai block.
 * 20-second delays to stay safe.
 */
...
// Delay between searches (ms) - 20 seconds to avoid rate limiting
  ...
  try {
    ...
    if (isBlocked) {
      return {
        query,
        queryIndex,
        surface: 'google-search-india-bangalore',
        success: false,
...
async function main() {
  ...
  console.log('  HUFT 100-Prompt Study - Google Search (India - Bangalore)');
  console.log('  Fresh IP pool with 20-second delays');
  console.log('='.repeat(70));
  console.log(`\nTotal queries: ${QUERIES.length}`);
  console.log(`Delay between searches: ${SEARCH_DELAY_MS / 1000} seconds`);
  console.log(`Estimated time: ~${Math.round(QUERIES.length * SEARCH_DELAY_MS / 1000 / 60)} minutes`);
  if (RESUME_FROM_QUERY > 0) {
    console.log(`Resuming from query: ${RESUME_FROM_QUERY + 1}`);
  }
  console.log('');

  // Launch browser with Bangalore proxy
  console.log('Launching browser with India (Bangalore) proxy...');
  ...
  try {
    ...
    if (ipInfo.country !== 'IN') {
      console.warn(`⚠️  Warning: IP is in ${ipInfo.country}, not India`);
    }
  } catch (e) {
  ...
  if (RESUME_FROM_QUERY > 0) {
    try {
      const intermediatePath = `studies/huft-google-india-blr-intermediate-${RESUME_FROM_QUERY}.json`;
  ...
  for (let i = RESUME_FROM_QUERY; i < QUERIES.length; i++) {
    ...
    if (result.success) {
      const aiStatus = result.aiOverview ? ' [AI Overview]' : '';
      ...
      if (consecutiveFailures >= 3) {
        console.log('\n' + '!'.repeat(70));
        console.log('  ⚠️  3 consecutive failures - saving and stopping');
        console.log('!'.repeat(70));
        break;
      }
    }
    ...
    if ((i + 1) % 10 === 0) {
      const intermediatePath = `studies/huft-google-india-blr-intermediate-${i + 1}.json`;
  ...
  console.log(`\nGoogle Search (India-Bangalore): ${successCount}/${results.length} successful`);
  console.log(`AI Overviews found: ${aiOverviewCount}`);
  console.log(`Total time: ${Math.round(totalTime / 1000 / 60)} minutes`);

  // Save final results
  const outputPath = 'studies/huft-google-india-bangalore-results.json';
  writeFileSync(outputPath, JSON.stringify({
    studyId: `huft-google-india-blr-${Date.now()}`,
    studyName: `${manifest.name} - Google Search (India-Bangalore)`,
    location: { city: 'Bangalore', region: 'Karnataka', country: 'IN', proxyType: 'residential' },
    timestamp: new Date().toISOString(),
