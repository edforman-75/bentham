#!/usr/bin/env npx tsx
/**
 * TASC Performance Visibility Study - OpenAI API Surface
 */
...
async function main() {
  ...
  console.log('\n' + '='.repeat(70));
  console.log('  TASC PERFORMANCE VISIBILITY STUDY - OpenAI GPT-4o API');
  ...
  for (let i = startIndex; i < QUERIES.length; i++) {
    ...
    if ((i + 1) % 10 === 0 || i === QUERIES.length - 1) {
      const output = {
        studyName: 'TASC Performance Visibility Study - OpenAI GPT-4o API',
        surface: 'openai-api',
