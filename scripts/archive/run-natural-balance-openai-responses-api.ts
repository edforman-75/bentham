#!/usr/bin/env npx tsx
/**
 * Natural Balance US Visibility Study - OpenAI Responses API Surface
 */

import { writeFileSync, existsSync, mkdirSync } from 'fs';

const OUTPUT_DIR = 'repository/results/glu/natural-balance-us-visibility';
const OUTPUT_FILE = `${OUTPUT_DIR}/openai-responses-api-results.json`;
...
async function callOpenAIResponses(query: string, apiKey: string): Promise<{ response: string; error?: string }> {
  try {
...
async function main() {
  ...
  console.log('\n' + '='.repeat(70));
  console.log('  NATURAL BALANCE US VISIBILITY STUDY - OpenAI Responses API');
  ...
  for (let i = 0; i < QUERIES.length; i++) {
    ...
    const { response, error } = await callOpenAIResponses(query, apiKey);
  ...
  const output = {
    studyName: 'Natural Balance US Visibility Study - OpenAI Responses API',
