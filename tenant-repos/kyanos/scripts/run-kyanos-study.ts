...
// Surfaces that require login (will prompt user)
const SURFACES_REQUIRING_LOGIN = new Set([
  'chatgpt-web',
  'claude-web',
  'perplexity-web',
  'gemini-web',
...
async function humanPause(page: Page, type: 'quick' | 'normal' | 'thinking' | 'between_queries'): Promise<void> {
  const delays: Record<string, [number, number]> = {
    ...
    normal: [800, 2000],
    thinking: [2500, 6000],
    between_queries: [8000, 15000], // 8-15 seconds between queries (slower to avoid bot detection)
  };
...
// ============================================================================
// Auto-Open Surface Tabs
// ============================================================================

/**
 * Opens browser tabs for all required surfaces and prompts for login if needed
 */
async function openSurfaceTabs(activeSurfaces: { id: string; name?: string }[]): Promise<void> {
  console.log('\n' + '─'.repeat(70));
  console.log('  OPENING BROWSER TABS FOR SURFACES');
  console.log('─'.repeat(70) + '\n');

  const browser = await chromium.connectOverCDP('http://localhost:9222');
  const context = browser.contexts()[0];

  const surfacesNeedingLogin: string[] = [];

  for (const surface of activeSurfaces) {
    const url = SURFACE_URLS[surface.id];
    if (!url) {
      console.log(`  ⚠️  ${surface.name || surface.id}: No URL configured, skipping`);
      continue;
    }

    // Check if tab already exists
    const pattern = SURFACE_PATTERNS[surface.id];
    const existingPage = context.pages().find(p => pattern.test(p.url()));

    if (existingPage) {
      console.log(`  ✅ ${surface.name || surface.id}: Tab already open`);
      continue;
    }

    // Open new tab
    console.log(`  🌐 ${surface.name || surface.id}: Opening ${url}...`);
    ...
    try {
      ...
      // Track if this surface needs login
      if (SURFACES_REQUIRING_LOGIN.has(surface.id)) {
        surfacesNeedingLogin.push(surface.name || surface.id);
      }
    } catch (error) {
  ...
  // If any surfaces need login, prompt user
  if (surfacesNeedingLogin.length > 0) {
    console.log('\n' + '─'.repeat(70));
    console.log('  ⚠️  LOGIN REQUIRED');
    console.log('─'.repeat(70));
    console.log('\n  The following surfaces may require login:');
    for (const name of surfacesNeedingLogin) {
      console.log(`    • ${name}`);
    }
    console.log('\n  Please log in to each service in the browser windows.');
    console.log('  Press Enter when you are logged in and ready to continue...\n');

    await new Promise<void>(resolve => {
      process.stdin.once('data', () => resolve());
    });
    console.log('  ✅ Continuing with study...\n');
  }
}
...
async function runSurfaceQueries(
  ...
  for (let batchStart = 0; batchStart < jobs.length; batchStart += batchSize) {
    ...
    // Delay between batches (human-like, slower to avoid bot detection)
    if (batchEnd < jobs.length) {
      const betweenDelay = randomDelay(10000, 20000); // 10-20 seconds between queries
      console.log(`  [${surfaceName}] ⏳ Waiting ${(betweenDelay / 1000).toFixed(0)}s before next query...`);
...
async function runStudy(manifest: Manifest): Promise<void> {
  ...
  // Auto-open browser tabs for all required surfaces
  await openSurfaceTabs(activeSurfaces);
